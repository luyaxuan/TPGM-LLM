...
class DataLoader(object):
    def __init__(self, xs, xt ,ys, batch_size, pad_with_last_sample=True):
        self.batch_size = batch_size
        self.current_ind = 0
        if pad_with_last_sample:
            num_padding = (batch_size - (len(xs) % batch_size)) % batch_size
            x_padding = np.repeat(xs[-1:], num_padding, axis=0)
            xt_padding = np.repeat(xt[-1:], num_padding, axis=0)  
            y_padding = np.repeat(ys[-1:], num_padding, axis=0)

            xs = np.concatenate([xs, x_padding], axis=0)
            xt = np.concatenate([xt, xt_padding], axis=0)  
    ...
    def shuffle(self):
        ...
        xs,  xt,ys = self.xs[permutation], self.xt[permutation], self.ys[permutation]
        self.xs = xs
        self.xt = xt  
        self.ys = ys
    ...
    def get_iterator(self):
        ...
        def _wrapper():
            while self.current_ind < self.num_batch:
                ...
                xt_i = self.xt[start_ind:end_ind, ...]  
...
def load_dataset(dataset_dir, batch_size, valid_batch_size=None, test_batch_size=None):
    ...
    for category in ["train", "val", "test"]:
        ...
        data["xt_" + category] = cat_data["xt"]

        data["x_" + category] = data["x_" + category][..., 1:] 
    ...
    data["xt_train"] = data["xt_train"][random_train, ...]  
    ...
    data["xt_val"] = data["xt_val"][random_val, ...]  
    ...
    data["train_loader"] = DataLoader(data["x_train"], data["xt_train"],data["y_train"], batch_size)
    data["val_loader"] = DataLoader(data["x_val"], data["xt_val"],data["y_val"], valid_batch_size)
    data["test_loader"] = DataLoader(data["x_test"], data["xt_test"],data["y_test"], test_batch_size)
    data["scaler"] = scaler
