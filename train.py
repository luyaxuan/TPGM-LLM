...
from util import *
import random
from model_TPGM_LLM import TPGM_LLM
...
parser.add_argument("--input_text_dim", type=int, default=768, help="input_dim")#文本嵌入特征维度
...
class trainer:
    def __init__(
        ...
        #self.loss2 = util.OrdinalCrossEntropy_torch
        self.scaler = scaler
        self.clip = 5
    ...
    def train(self, input, input_xt, real_val):
        self.model.train()
        self.optimizer.zero_grad()
        output = self.model(input,input_xt)#input (64,15,1296,2),input_xt (64, 768, 15)
        output = output.transpose(1, 3)#(64,2,1296,15)
        real = real_val
    ...
    def eval(self, input , input_xt , real_val):
        self.model.eval()
        output = self.model(input,input_xt)
...
def main():
    ...
    for i in range(1, args.epochs + 1):
        ...
        for iter, (x,xt, y) in enumerate(dataloader["train_loader"].get_iterator()):
            trainx = torch.Tensor(x).to(device)#(64,15,1296,4)
            trainx = trainx.transpose(1, 3)#(64,4,1296,15)
            trainy = torch.Tensor(y).to(device)#(64,15,1296,2)
            trainy = trainy.transpose(1, 3)#(64,2,1296,15)
            trainxt = xt
            metrics = engine.train(trainx, trainxt , trainy)
        ...
        s1 = time.time()
        for iter, (x, xt , y) in enumerate(dataloader["val_loader"].get_iterator()):
            testx = torch.Tensor(x).to(device)
            testx = testx.transpose(1, 3)
            testy = torch.Tensor(y).to(device)
            testy = testy.transpose(1, 3)
            testxt = xt
            metrics = engine.eval(testx,testxt, testy)
        ...
        if mvalid_loss < loss:
            ...
            if i < 5:
                ...
                for iter, (x,xt, y) in enumerate(dataloader["test_loader"].get_iterator()):
                    testx = torch.Tensor(x).to(device)
                    testx = testx.transpose(1, 3)
                    testxt = xt
                    with torch.no_grad():
                        preds = engine.model(testx,testxt).transpose(1, 3)
